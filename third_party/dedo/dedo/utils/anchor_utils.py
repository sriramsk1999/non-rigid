...
def command_anchor_position(sim, anchor_bullet_id, tgt_pos, tax3d=False, task='proccloth'):
    """
        tax3d: if True, use modified control to bias towards target position
        task: DEDO task, e.g. 'proccloth' or 'hangbag'
    """
    anc_pos, _ = sim.getBasePositionAndOrientation(anchor_bullet_id)
    anc_linvel, _ = sim.getBaseVelocity(anchor_bullet_id)
    ...
    sim.applyExternalForce(
        anchor_bullet_id, -1, force.tolist(), [0, 0, 0], pybullet.LINK_FRAME)
    # print(np.linalg.norm(pos_diff))
    # if tax3d:
    #     if task == 'proccloth':
    #         # bias towards target position
    #         raw_force = 35.0 * vel_diff + CTRL_PD_KD_POS * pos_diff
    #     elif task == 'hangbag':
    #         # bias towards target position
    #         raw_force = 35.0 * vel_diff + CTRL_PD_KD_POS * pos_diff
    #         # TODO: floor the pos_diff?
    #     else:
    #         raise ValueError('Unknown task')
    # else:
    #     raw_force = CTRL_PD_KD * vel_diff + CTRL_PD_KD_POS * pos_diff
