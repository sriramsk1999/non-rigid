...
def gen_procedural_hang_cloth(args, preset_obj_name, deform_info_dict, deform_params={}):
    '''
    ...
    if 'node_density' in deform_params:
        ...
        deform_params['node_density'] = node_density
    
    if 'num_holes' in deform_params:
        num_holes = deform_params['num_holes']
    else:
        num_holes = args.num_holes
        deform_params['num_holes'] = num_holes
    
    if 'w' in deform_params:
        w = deform_params['w']
    else:
    ...
    if 'holes' in deform_params:
        ...
        # hole generation
        constraints = {}
        constraints['x_range'] = (2, node_density - 2)
        constraints['y_range'] = (2, node_density - 2)
        # constraints['width_range'] = (1, int(round(node_density * 0.3)))
        # constraints['height_range'] = (1, int(round(node_density * 0.3)))
        constraints['width_range'] = (5, 7)
        constraints['height_range'] = (5, 7)
    ...
    cloth_obj_path, cloth_anchor_indices, gt_loop_vertices = create_cloth_obj(
        ...
        node_density=node_density, #args.node_density,
        holes=holes,
        data_path=savepath,
    )
    ...
    return args.deform_obj, deform_params
