...
def play(env, num_episodes, args):
    for epsd in range(num_episodes):
        ...
        ids = [1, 2, 16777217, 16777218, 33554433]

        n_vert, verts = p.getMeshData(env.deform_id, flags=p.MESH_DATA_SIMULATION_MESH)
        verts = np.array(verts)
        rigid_verts = []
        for rid in ids:
            n_vert, verts = p.getMeshData(rid)
            rigid_verts.append(np.array(verts))
        rigid_verts = np.concatenate(rigid_verts)
        fig = vpl.segmentation_fig(rigid_verts, np.ones(rigid_verts.shape[0], dtype=np.int64))
        fig.show()
        quit()
        ...
        while True:
            ...
            if done:
                # get final pcd
                # pcd_obs = env.get_pcd_obs()
                # img_final, pcd_final, ids_final = pcd_obs.values()
                n_verts, verts2 = p.getMeshData(env.deform_id, flags=p.MESH_DATA_SIMULATION_MESH)
        ...
        fig = vpl.segmentation_fig(verts2, np.ones(verts2.shape[0], dtype=np.int64))
        fig.show()
        # if args.pcd:
