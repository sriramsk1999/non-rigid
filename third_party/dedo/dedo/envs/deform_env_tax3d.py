...
class DeformEnvTAX3D(gym.Env):
    ...
    def __init__(self, args):
        ...
        self.gripper_lims = np.tile(np.concatenate(
            [DeformEnvTAX3D.WORKSPACE_BOX_SIZE * np.ones(3),  # 3D pos
        ...
        self.action_space = gym.spaces.Box(  # [-1, 1]
            ...
            2.0 * np.ones(self.num_anchors * act_sz))
        if self.args.debug:
            print('Created DeformEnvTAX3D with obs', self.observation_space.shape,
    ...
    def unscale_vel(act, unscaled):
        if unscaled:
            return act
        return act*DeformEnvTAX3D.MAX_ACT_VEL
    ...
    def load_objects(self, sim, args, debug, 
        ...
        if self.camera_config is not None:
            ...
            if rigid_trans is not None:
                # for id, (name, kwargs) in zip(rigid_ids, SCENE_INFO[scene_name]['entities'].items()):
    ...
    def reset(self, cloth_rot=None, 
        ...
        if self.rollout_vid:
            ...
            # TODO: ALSO UPDATE THE COLOR KEY HERE
    ...
    def do_action_velocity(self, action, unscaled):
        ...
        for i in range(self.num_anchors):
            command_anchor_velocity(
                ...
                DeformEnvTAX3D.unscale_vel(action[i], unscaled))
    ...
    def make_final_steps(self):
        ...
        info = {'final_obs': []}
        for sim_step in range(DeformEnvTAX3D.STEPS_AFTER_DONE):
            ...
            if self.args.task.lower() == 'lasso':
                if sim_step % self.args.sim_steps_per_action == 0:
                    action = [10*DeformEnvTAX3D.MAX_ACT_VEL,
                              10*DeformEnvTAX3D.MAX_ACT_VEL, 0]
    ...
    def get_grip_obs(self):
        ...
        for i in range(self.num_anchors):
            ...
            anc_obs.extend(pos)
            anc_obs.extend((np.array(linvel)/DeformEnvTAX3D.MAX_OBS_VEL))
        return anc_obs
    ...
    def get_reward(self, debug=False):
        ...
        for i in range(num_holes_to_track):  # loop through goal vertices
            ...
            if len(cent_pts) == 0 or np.isnan(cent_pts).any():
                dist = DeformEnvTAX3D.WORKSPACE_BOX_SIZE*num_holes_to_track
                dist *= DeformEnvTAX3D.FINAL_REWARD_MULT
        ...
        if self.args.env.startswith('HangProcCloth'):
            ...
            dist = np.mean(dist)
        rwd = -1.0 * dist / DeformEnvTAX3D.WORKSPACE_BOX_SIZE
        return rwd
