...
if __name__ == "__main__":
    ...
    NUM_TRAINING_DEMOS_PER_OBJ = 64
    NUM_VAL_DEMOS_PER_OBJ = 32
    ...
    NR_DIR = Path(os.path.expanduser(f"~/datasets/nrp/dataset/demos/revolute-3_prismatic-3_flow"))
    GUI = False
    # USE_EGL = not GUI
    USE_EGL = False
    GENERATE_TRAIN = True
    ...
    GOAL_MAP = {
        "door": lambda: np.random.rand() * np.pi / 2,
        "drawer": lambda: np.random.rand() * 0.5,
    }
    ...
    if GENERATE_TRAIN:
        ...
        for i in range(len(OBJ_IDs)):
            env = PMObjectEnv(OBJ_IDs[i][0], PM_DIR, GUI, use_egl=USE_EGL, link_semantics=OBJ_IDs[i][1])
            for j in range(NUM_TRAINING_DEMOS_PER_OBJ):
                print(f'Generating OBJ {OBJ_IDs[i]} training demo {j}...')
                # sampling random goal
                goal = GOAL_MAP[OBJ_IDs[i][1]]()
                if OBJ_IDs[i][1] == "door":
                    print(f'Goal: {goal * 180 / np.pi} degrees')
                elif OBJ_IDs[i][1] == "drawer":
                    print(f'Goal: {goal} meters')
