...
from non_rigid.models.df_base import DiffusionFlowBase, ClothFlowTrainingModule
...
def predict_wta(network, batch, device, diffusion, num_wta_trials=50, sample_size=213):
...
def main(cfg):
    ...
    for batch in tqdm(val_loader):
        ...
        # pred_pc = pred_pc.reshape(-1, 213, 3)
    ...
    # pred_seg = np.array([np.arange(1, 161)] * 213).T.flatten()
    ...
    # adding traces for final destination to keep everything in frame
    pcd_final = pcd + results[-1].squeeze(0).permute(1, 0).cpu()
    flows_final = torch.zeros_like(pcd_final)
    combined_pcd = torch.cat([pcd, pcd_final], dim=0)


    for noise_step in tqdm(results[0:]):
        pred_flow_step = noise_step.squeeze(0).permute(1, 0).cpu()
        print(pred_flow_step.shape)

        combined_flow = torch.cat([pred_flow_step, flows_final], dim=0)
        ...
        #     "red",
        # )
        # ADD TRACE FOR FINAL POINT CLOUD
